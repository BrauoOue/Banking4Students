...
export default function Index() {
    ...
    const creditCards = [
        ...
        { id: "2", bank: "MasterCard", number: "**** 5678", balance: "$1,875.20", color: "bg-blue-700" },
        { id: "3", bank: "Amex", number: "**** 9876", balance: "$5,340.75", color: "bg-green-600" },
    ];
    ...
    const requests = [
        { id: "1", from: "Nikola J.", amount: "-$9.99", date: "Feb 25", reason: "Restaurant Bill" },
        { id: "2", from: "Gorazd F.", amount: "-$2.99", date: "Feb 30", reason: "Srekja Bar" },
        { id: "3", from: "Andrea S.", amount: "-$11.11", date: "Jan 1", reason: "Public Room" },
    ];
    ...
    return (
        <ScrollView className="flex-2 bg-white p-5">
            {/* Title */}
            <Text className="text-xl font-bold text-text mb-5">Your Credit/Virtual Cards</Text>
            ...
            <View className="flex-row justify-between my-5">
                {["Send", "Request", "My Uni", "Cash Stuffing"].map((action, index) => (
                    <TouchableOpacity key={index} className="align-center justify-center bg-accent p-3 rounded-lg w-20 items-center shadow-md shadow-accent">
                        <Text className="text-white text-sm font-semibold text-center">{action}</Text>
                    </TouchableOpacity>
                ))}
            </View>

            {/* Transaction History */}
            <Text className="text-lg font-bold text-text mt-4">Recent Transactions</Text>
            <View className="p-3 rounded-lg">
                {transactions.map((transaction) => (
                    <View key={transaction.id} className="flex-row justify-between border-b border-gray-300 py-3">
                        ...
                        <Text className={`font-semibold ${transaction.type === "credit" ? "text-green-600" : "text-red-500"}`}>
                            {transaction.amount}
                        </Text>
                    </View>
                ))}
                    ...
                    <View className="w-40 h-28 bg-gray-100 p-3 m-2 rounded-lg shadow-md">
                        ...
                        <View className="flex-row justify-between mt-2">
                            <TouchableOpacity className="bg-green-500 px-3 py-1 rounded-md">
                                <Image source={icons.accept} className="w-5 h-5" resizeMode="contain" />
                                {/*<Text className="text-white font-bold">Accept</Text>*/}
                            </TouchableOpacity>
                            <TouchableOpacity className="bg-red-500 px-3 py-1 rounded-md">
                                <Image source={icons.deny} className="w-5 h-5" resizeMode="contain" />
            ...
            <View className="bg-accent p-4 rounded-lg shadow-lg shadow-accent">
                <Text className="text-lg font-bold text-white">{partnership.title}</Text>
                <Text className="text-white text-sm mt-1">{partnership.desc}</Text>
                <View className="flex-row items-center mt-2">
                    <Image source={icons.coins} className="w-5 h-5" resizeMode="contain" />
                    <Text className="text-lg font-bold text-yellow-500 ml-2">{partnership.amount} Points</Text>
                </View>
            </View>
            <TouchableOpacity className="bg-primary p-3 mt-3 mb-[15vh] rounded-lg items-center shadow-md">
